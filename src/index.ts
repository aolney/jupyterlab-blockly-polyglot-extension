...
export class BlocklyWidget extends Widget {
   ...
   * @returns 
   */
  createDummyNode(doc: Document, n: Node, diff: ChangeObject<string>) {
    //id
    let id = this.generateRandomString(10);
    //position
    let y = 0;
    let old_y = (n as Element).getAttribute("y")?.valueOf();
    if (old_y) y = parseInt(old_y) - 30 //add offset
    let old_x = (n as Element).getAttribute("x")?.valueOf() ?? "0";
    //code is actual for add and comment for remove
    let code = diff.added ? diff.value : "#" + diff.value;
    //parse xml templated with above info
    let new_xml = this.domParser.parseFromString(`<block type="dummyNoOutputCodeBlock" id="${id}" x="${old_x}" y="${y}"><field name="CODE">${code.trim()}</field></block>`, "text/xml");
    //first child of doc is the <block>
    return new_xml.childNodes.item(0);
  }

  /**
   * EXPERIMENTAL - NOT WORKING. NOTE: When we insert freestyles, the x/y coordinates seem to be 
   * affected by existing blocks. This suggests that we need to re-layout all blocks. Additionally,
   * we have complexity with connected vs unconnected blocks (in rows). This is sufficiently complicated
   * and buggy that I'm not implementing it at this time
   * ------------------------------------------------------------
      ...
      if (xmlString != null) {
        try {
          ...
          //we have diffs and need an alternative decoding procedure
            ...
            // remove <next> and </next> tags to normalize traversal
            let clean_xml = user_blocks_xml.replace("<next>", "").replace("</next>", "");
            ...
            let blockCount = 1;
            let diffIndex = 0;
            // let diffLimit = diffs.reduce((accumulator, diff : ChangeObject<string>) => accumulator + diff.count, 0);
            ...
            for (let i = 0; i < xml_root.childNodes.length || diffIndex < diffs.length; i++) {
              ...
              if (n.nodeName == "block") {
                ...
                if (blockCount >= diffs[diffIndex].count) {
                  //if no add and no remove, then no op but update indices
                  if (!diffs[diffIndex].added && !diffs[diffIndex].removed) {
                    diffIndex++;
                    blockCount = -1;
                  } else {
                    //make node; either add or remove
                    let new_node = this.createDummyNode(doc, n, diffs[diffIndex]);
                    //insert if diff nonempty - NOTE: BLOCKLY OVERRIDING MY LAYOUT!
                    let next = n;// xml_root.childNodes.item(i);// + 1);
                    if( diffs[diffIndex].value.trim() != "" ) {
                      xml_root.insertBefore(new_node, next); //where it is in the tree seems irrelevant since the y coord determines position
                    }
                    //update indices
                    diffIndex++;
                    blockCount = -1;
                ...
                blockCount += 2; //normalization adds blank lines so include those in count
              }
              // console.log(n);
              // console.log(diffs[i]);
            }
