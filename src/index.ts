...
export class BlocklyWidget extends Widget {
   ...
   * @returns 
   */
  createDummyNode(n:Node, diff : ChangeObject<string>){
    //make node
    let new_node = n.cloneNode();
    let element = new_node as Element;
    let id = this.generateRandomString(10);
    element.setAttribute("id",id);
    let y = 0;
    let y_attr = element.getAttribute("y")?.valueOf();
    if( y_attr ) y = parseInt(y_attr) + 10 //add offset
    element.setAttribute("y",y.toString());
    //TODO get template from toolbox for better encapsulation
    // diff.add -> add code; diff.remove -> comment code
    let code  = diff.added ? diff.value : "#" + diff.value;
    new_node.nodeValue = `<block type="dummyNoOutputCodeBlock" id="${id}" x="-543" y="${y}"><field name="CODE">${code}</field></block>`;
    return new_node;
  }
    ...
    if (this.notebooks.activeCell) {
      ...
      if (xmlString != null) {
        try {
          ...
          //the simplest thing to do is take the diff and add new blocks to the end of the XML
          //putting new blocks in the right diff position would require a mapping between the XML and associated code,
          //which AFAIK would require full DOM parsing
          else {
            ...
            for(let i = 0; i < xml_root.childNodes.length || (i < diffLimit && diffIndex < diff.length); i++){
              ...
              if( n.nodeName == "block"){
                ...
                if( blockCount >= diffs[diffIndex].count){
                  ...
                  if(!diffs[diffIndex].added && !diffs[diffIndex].removed) {
                    ...
                    let new_node = this.createDummyNode(n, diffs[diffIndex]);
                    //insert
                    let next = xml_root.childNodes.item(i + 1);
                    xml_root.insertBefore(new_node, next);
                    //update indices
                    diffIndex++;
                    blockCount = -1;
                  }
