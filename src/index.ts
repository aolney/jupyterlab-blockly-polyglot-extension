...
import { toolbox as toolbox_r } from './Toolbox-r'
...
class BlocklyWidget extends Widget {
  ...
  constructor(notebooks: INotebookTracker) {
    ...
    this.node.appendChild(this.createLanguageSwitcher());
  ...
  createLanguageSwitcher(): HTMLElement {
  ...
  onResize = (msg: Widget.ResizeMessage): void => {
    ...
    blocklyDiv.setAttribute("style", ((("width: " + ((copyOfStruct = msg.width, copyOfStruct.toString()))) + "px; height: ") + (adjustedHeight - 20).toString()) + "px");
...
async function activate(app: JupyterFrontEnd, palette: ICommandPalette, notebooks: INotebookTracker, restorer: ILayoutRestorer | null, panel: NotebookPanel) {
  console.log('jupyterlab_blockly_extension: extension is activated!');
